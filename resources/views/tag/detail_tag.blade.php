...
<div class="container-fluid">
    <div class="d-flex justify-content-between">
        <h1>Seluruh pertanyaan dengan tag #{{$tags->title ?? $tag}}</h1>
        @if(Auth::check())
    ...
    @if($tags)
    <form action="/tag/{{$tags->title ?? $tag}}" method="get">
    ...
    @endif
    @if($tags)
    @foreach($tags->pertanyaan as $tanya)
        <div class="card">
    ...
    @if($tags == null)
        <div class="card shadow">
            <div class="card-body">
                <p class="card-text">Masih belum ada yang menanyakan masalah dengan tag ini.
                    @if(Auth::check())
